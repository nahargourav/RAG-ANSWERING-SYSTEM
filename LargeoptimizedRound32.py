...
async def answer_question_with_rag(question: str, chunks: list[str], faiss_index: faiss.Index, doc_url: str) -> str:
    ...
    # Before adding to FAISS or returning from search:
...
Use the following context chunks to answer the question. If the document **partially answers** the question, provide what is known and infer reasonable answer to the question based on know things.

Only say "Information not available in the document." if the context is truly unrelated or insufficient.

When the question combines multiple parts (e.g. benefits + process + eligibility), answer all parts based on the context.

Be concise, factual, and do not speculate beyond the document.

provide concise answer in 2-3 sentences.
